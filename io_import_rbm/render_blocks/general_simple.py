...
def process_block(filepath, file, imported_objects):
    print(f"Processing GeneralSimple block from {filepath}")
    ...
    if material is None:
        ...
        node_group = bpy.data.node_groups.get("GeneralSimple")
        if not node_group:
            print("Node group 'GeneralSimple' not found.")
        else:
