...
def process_block(filepath, file, imported_objects):
    ...
    material_data = [read_float(file) for _ in range(70)]
    ...
    for i in range(vertcount):
        ...
        if flags & IS_SKINNED:
            read_u32(file)  # boneweight
            read_u32(file)  # boneindex
    ...
    # If DESTRUCTION or IS_SKINNED flag is true, process remap section
    if flags & DESTRUCTION or flags & IS_SKINNED:
        file.seek(file.tell() + 12)
        remapcount = read_u32(file)
        print(f"Remap Count: {remapcount}")
        
        remaps = [read_u16(file) for _ in range(remapcount)]
        print("Remaps:", remaps)
    ...
    if material is None:
        ...
        node_group = bpy.data.node_groups.get("GeneralMK3")
        if not node_group:
            print("Node group 'GeneralMK3' not found.")
        else:
        ...
        # Set floats
        for i, float_value in enumerate(material_data[:70]):
            input_index = i + 13
            if input_index < len(shader_node.inputs):
        ...
        for texture_number, texture_path in enumerate(filepaths, start=1):
            ...
            tex_node.location = (-300, -100 * (input_index - 80))  # Adjust location for neat arrangement
            ...
            uv_node.location = (-500, -100 * (input_index - 80))  # Adjust UV node location
