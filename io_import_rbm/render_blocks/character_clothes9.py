...
def process_block(filepath, file, imported_objects):
    ...
    renderblocktype = "CharacterClothes9" 
    if filepaths:
    ...
    if material is None:
        ...
        node_group = bpy.data.node_groups.get("CharacterClothes9")
        if not node_group:
            print("Node group 'CharacterClothes9' not found.")
        else:
        ...
        TEXTURE_SETTINGS = {
            ...
            "uv2": [4, 5],
            "srgb": [1, 4, 7,],
            "non_color": [2, 3, 5, 6, 8, 9, 11],
        }

        input_index = 8  # Start at input index 83 to skip the first 83 inputs (this number is booleans+floats)
        ...
        for texture_number, texture_path in enumerate(filepaths, start=1):  # Start at 1 for Blender indexing
            ...
            tex_node.location = (-300, -100 * (input_index))  # Subtract 80 for location offset
