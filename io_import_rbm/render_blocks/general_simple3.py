...
def process_block(filepath, file, imported_objects):
    ...
    renderblocktype = "GeneralSimple3"  
    if filepaths:
    ...
    if material is None:
        ...
        node_group = bpy.data.node_groups.get("GeneralSimple3")
        if not node_group:
            print("Node group 'GeneralSimple3' not found.")
        else:
